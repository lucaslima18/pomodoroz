...
class CLIManager:
    ...
    def main_logo(self):
        ...
        print(
            "🍅 An extremely minimalist Command-line Pomodoro Timer for Linux Systems 🚀\n"
        )
    ...
    def paginated_choice(
        self, message: str, offset: int, limit: int, task_list: List[str]
    ) -> str:
        ...
        task = questionary.select(message=message, choices=page_content).ask()
    ...
    def create_pomodoro(self, pomodoro_manager: PomodoroManager):
        for index, _ in enumerate(pomodoro_manager.pomodoro_progress):
            ...
            progress_bar(
                total_seconds=pomodoro_manager.pomodoro_info.time_for_pomodoro_sec,
                state="pomodoro",
            )
            ...
            if index < pomodoro_manager.pomodoro_info.count - 1:
                ...
                self.create_break(
                    pomodoro_manager=pomodoro_manager, act_pomodoro_index=index
                )
    ...
    def create_break(self, pomodoro_manager: PomodoroManager, act_pomodoro_index: int):
        self.pomodoro_menu(pomodoro_manager=pomodoro_manager)
        break_time = pomodoro_manager.pomodoro_progress[act_pomodoro_index][
            "break_time_sec"
        ]
