...
class MenuManager:
    def __init__(self, app_name: str, app_icon: str, storage_path: str) -> None:
        self.notifiction_manager = NotificationManager(
            app_name=app_name, app_icon=app_icon, storage_path=storage_path
        )
    ...
    def __project_task_select(self, offset: int, limit: int):
        ...
        project = self.cli_manager.paginated_choice(
            message="How task you want start?",
            offset=offset,
            limit=limit,
            task_list=task_fake_list,
        )
