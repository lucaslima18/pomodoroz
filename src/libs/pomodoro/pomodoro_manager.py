...
class PomodoroManager:
    ...
    def show_pomodoro(self):
        ...
        print(f"🕒 Total time: {timedelta(seconds=self.pomodoro_info.time_for_pomodoro_sec*self.pomodoro_info.count)}m\n\n")
        print(self.motivational_phrase() + "\n\n")
    
    def show_pomodoro_tomatoes(self):
        try:
            return ''.join("🍏" if pomodoro['done'] else "🍅" for pomodoro in self.pomodoro_progress)
    ...
    def __progress_calculate(self) -> List[PomodoroProgress]:
        ...
        try:
            if  self.pomodoro_info.count > 4:
                breaks_without_addition = [{'done': False, 'break_time_sec': default_break_time_sec} for _ in range(3)] 
                breaks_with_addition = [{'done': False, 'break_time_sec': default_break_time_sec+(break_addition*default_break_time_sec)} for break_addition in range(self.pomodoro_info.count-3)]
                return breaks_without_addition + breaks_with_addition
            return [{'done': False, 'break_time_sec': default_break_time_sec} for _ in range(self.pomodoro_info.count)]
        except Exception as err:
            print(err)